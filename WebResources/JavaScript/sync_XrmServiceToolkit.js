        ...
        function init(data) {
            ...
            for (var i = 0, ilength = parentFields.length; i < ilength; i++) {
                ...
                for (var a = 0, alength = options.length; a < alength; a++) {
                    var option = {};
                    option.value = jQuery(parseInt(options[a]).attr("value"));
                    option.showOptions = [];
                    var optionsToShow = jQuery(options[a]).children("ShowOption");
                    for (var b = 0, blength = optionsToShow.length; b < blength; b++) {
                        var optionToShow = {};
                        optionToShow.value = jQuery(parseInt(optionsToShow[b])).attr("value");
                ...
                if ((dependentOptionSet.parent === parentField) && (dependentOptionSet.dependent === childField)) {
                    ...
                    for (var os in dependentOptionSet.options) {
                        ...
                        if (parent.getValue().toString() === options.value.toString()) {
							...
							if (!bCurrentValueIsValid && optionsToShow.length==1)
							{
								bCurrentValueIsValid=true;
								currentChildFieldValue = parseInt( childFieldOptions[0].value)
							}
                            /*
...
if (typeof (SoftPro) == "undefined")
	{ SoftPro = function(){}; }

	SoftPro.CopyEntity = function(){
		var getObjectTypeCode = function(entityName) 
		{
			try {
		...
		{
//			debugger;
			var formLabel = Xrm.Page.ui.formSelector.getCurrentItem();
			var EntityCopierRecord = getEntityCopierRecord(getObjectTypeCode(Xrm.Page.data.entity.getEntityName()),formLabel.getId());
			if (EntityCopierRecord!=null)
			{
				var fields=getEntityCopierFields(EntityCopierRecord.id);
				 if (fields!=null && fields.length > 0) 
				{
					var params={};
					params["formid"]=formLabel.getId();
					for(i=0;i<fields.length;i++)
					{
						if (Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value) && Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value).getValue()!=null &&
							Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value).getValue()!='')
							{
								if (Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value).getAttributeType()=="lookup" )
									{
										params[fields[i].attributes["softpro_name"].value]=Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value).getValue()[0].id;
										params[fields[i].attributes["softpro_name"].value+"name"]=Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value).getValue()[0].name;
										if (fields[i].attributes["softpro_name"].value=="ownerid" || fields[i].attributes["softpro_name"].value=="customerid")
										params[fields[i].attributes["softpro_name"].value+"type"]=Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value).getValue()[0].typename;
									}
									else
										params[fields[i].attributes["softpro_name"].value]=Xrm.Page.getAttribute(fields[i].attributes["softpro_name"].value).getValue();
							}
					}					
		...
		var getEntityCopierFields=function (id)
		{
			var xml='<fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">'
				+ ' <entity name="softpro_entitycopierfields">'
				+'    <attribute name="softpro_name" />'
				+'    <filter type="and">'
				+'      <condition attribute="softpro_entitycopier" operator="eq" value="' + id + '" />'
				+'    </filter>'
				+'  </entity>'
				+'</fetch>';
				var  res = XrmServiceToolkit.Soap.Fetch(xml);
            return res;
		};
		var getEntityCopierRecord=function(entityOtc,FormId)
		{
			var xml='<fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">'
				+ ' <entity name="softpro_entitycopier">'
				+'    <attribute name="softpro_entitycopierid" />'
				+'    <filter type="and">'
				+'      <condition attribute="softpro_entity" operator="eq" value="' + entityOtc + '" />'
				+'      <condition attribute="softpro_form" operator="eq" value="'+ FormId +'" />'
				+'    </filter>'
				+'  </entity>'
				+'</fetch>';
				var  res = XrmServiceToolkit.Soap.Fetch(xml);
